...
<section class="dashboard-shell exams-shell">
    ...
    <div class="metric-grid">
        <article class="metric"><span>Questoes</span><strong><?= e((int) $exam['questions_count']) ?></strong></article>
    ...
    <section class="detail-card">
        ...
        <p><strong>Janela:</strong> <?= e($exam['starts_at'] ? date('d/m/Y H:i', strtotime($exam['starts_at'])) : 'inicio livre') ?> ate <?= e($exam['ends_at'] ? date('d/m/Y H:i', strtotime($exam['ends_at'])) : 'sem fim definido') ?></p>

        <?php if ((int) $exam['attempts_used'] >= (int) $exam['attempts_allowed']): ?>
            <div class="flash info">Voce ja usou todas as tentativas disponiveis.</div>
        <?php else: ?>
